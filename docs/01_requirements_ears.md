...
### 5.4 電車の駅選択（自動/固定/一時切替）
（定数）
- `SWIPE_OVERRIDE_DURATION = 30 minutes`

- **SYS-REQ-030（Ubiquitous）**  
  システムは、電車表示対象駅を、以下の優先順位で決定 **しなければならない**。  
  1) 一時選択（Widget操作による切替で設定され、有効期限内）  
  2) 固定駅（設定画面）  
  3) 自動最寄り駅（現在地から直線距離最短）  
  検証: Test

- **SYS-REQ-031（Event-driven / When）**  
  **ユーザーがWidget上の駅切替操作を行ったとき**、システムは一時選択駅を更新し、その有効期限を **現在時刻+30分** に設定 **しなければならない**。  
  検証: Demonstration + Test

- **SYS-REQ-032（State-driven / While）**  
  **一時選択の有効期限内である間**、システムは電車表示対象駅として一時選択駅を優先 **しなければならない**。  
  検証: Test

- **SYS-REQ-033（Event-driven / When）**  
  **一時選択の有効期限を過ぎたとき**、システムは電車表示対象駅の決定を SYS-REQ-030 の優先順位に従って再評価 **しなければならない**。  
  検証: Test

> 注: Android Widgetでの“スワイプ”は標準的に困難なため、v1では「前へ/次へ」等のボタン操作で代替する（UI要求で規定）。
...
- **UI-REQ-002（Ubiquitous）**  
  Widgetは、駅切替のための操作要素（例: 前/次ボタン、またはタイトルタップで切替）を提供 **しなければならない**。  
  検証: Demonstration
