...
1. `./gradlew test` でユニットテストを実行し、全件パスを確認する
2. 実機が接続されている場合は `./gradlew installDebug` で実機に書き込む
   - 接続確認: `~/Library/Android/sdk/platform-tools/adb devices` でデバイスが表示されるか確認する
   - デバイスが見つからない場合はスキップし、その旨を報告する
